...
void SignIn::writeBinary() {
    ...
    binarySignInFile.write(reinterpret_cast<char *> (&information), sizeof(information));
    binarySignInFile.close();
}
...
void SignIn::validateUser() {
    ...
    while (binarySignInFile.read(reinterpret_cast<char *> (&temp), sizeof(temp))) {
        if(temp.username == information.username && temp.password == information.password){
