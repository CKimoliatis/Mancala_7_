        ...
        while (userDB.read(reinterpret_cast<char *> (&temp), sizeof (temp))) {
            ...
            if (validUsername && validPassword) {
                ...
                do {
                    ...
                    int passwordLength = 0;
                    char t = tryAgain[passwordLength];
                    while (t != '\0') {
                        passwordLength++;
                        t = tryAgain[passwordLength];
                    }

                    for (int i = 0; i < max(passwordLength, temp.passwordLength); i++) {
                        if (temp.password[i] != tryAgain[i]) {
                            validPassword = 0;
                        }
                    }
                } while (!validPassword);
                for (int i = 0; i < temp.passwordLength; i++) {
                    info.password[i] = temp.password[i];
                }
                info.passwordLength = temp.passwordLength;
                write = 0;
            }
        }
        userDB.clear();
